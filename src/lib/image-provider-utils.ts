...
function inferMediaRelayProtocol(provider: Pick<ImageProviderConfigShape, 'base_url'>): MediaRelayProtocol {
  ...
  if (baseUrl.includes('api.whatai.cc')) return 'openai-images';
...
function joinApiPath(baseUrl: string, endpoint: string): string {
  ...
  if (!cleanedBase) return cleanedEndpoint ? `/${cleanedEndpoint}` : '';
  if (!cleanedEndpoint) return cleanedBase;
...
function inferOpenAiImagesEndpoint(baseUrl: string): string {
  ...
  if (!normalized) return '/v1/images/generations';
  if (/\/v1\/images\/generations\/?$/i.test(normalized)) return normalized;
  if (/\/v1\/?$/i.test(normalized)) return joinApiPath(normalized, 'images/generations');
...
export function getMediaRelayProtocol(provider: Pick<ImageProviderConfigShape, 'base_url' | 'options_json'>): MediaRelayProtocol {
  ...
  return protocol === 'openai-images' ? protocol : inferMediaRelayProtocol(provider);
}
